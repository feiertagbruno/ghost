...
DECLARE @DATA_REFERENCIA VARCHAR(10); SET @DATA_REFERENCIA =  CONVERT(varchar,:data_referencia,112);
DECLARE @DATA_INICIAL VARCHAR(10); SET @DATA_INICIAL = CONVERT( VARCHAR, DATEADD(YEAR,{qtd_anos_atras}, @DATA_REFERENCIA),112);
WITH
FRETES AS (
	SELECT 
		[NF ORI], [SERIE NF],
...
def get_query_ultimo_fechamento_produtos():
	...
	DECLARE @DATA_INICIAL VARCHAR(10) = CONVERT(VARCHAR, DATEADD(YEAR,{qtd_anos_atras},:data_referencia),112);
...
def get_query_detalhamento_op():
	return """
DECLARE @NUMERO_OP VARCHAR(MAX) = :numero_op ;

WITH CONSULTA_OP AS (
	SELECT
		TRIM(D4_OP) op,
		TRIM(D4_PRODUTO) codigo_original,
		TRIM(B1A.B1_DESC) descricao_cod_original,
		B1A.B1_TIPO tipo_original,
	...
	INNER JOIN VW_MN_SB1 B1
		...
		AND D4_COD = B1_COD
		AND B1_TIPO <> 'MO'
	LEFT JOIN VW_MN_SB1 B1A
		ON B1A.D_E_L_E_T_ <> '*'
		AND B1A.B1_COD = D4_PRODUTO

	WHERE D4.D_E_L_E_T_ <> '*'
		AND D4_OP IN (SELECT value FROM string_split(@NUMERO_OP, ','))
)

SELECT 
	op, codigo_original, descricao_cod_original, tipo_original,
	data_referencia,
...
GROUP BY op, codigo_original, 
	descricao_cod_original, tipo_original,
	data_referencia,
	data_encerramento_op, insumo,
	descricao_insumo, tipo_insumo
"""
...
def get_query_numeros_op_por_periodo_somente_PA():
	return """
DECLARE @DATA_INICIAL VARCHAR(10) = CONVERT(VARCHAR, :data_inicial,112) ;
DECLARE @DATA_FINAL VARCHAR(10) = CONVERT(VARCHAR, :data_final, 112) ;
DECLARE @CODIGOS VARCHAR(MAX) = :codigos ;
